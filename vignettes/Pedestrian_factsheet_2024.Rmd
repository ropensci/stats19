...
This document aims to reproduce the <!-- `r yr2calc-4`-`r yr2calc` --> 2020-2024 sections of the ["Accredited official statistics Reported road casualties in Great Britain: pedestrian factsheet, 2024"](https://www.gov.uk/government/statistics/reported-road-casualties-great-britain-pedestrian-factsheet-2024/reported-road-casualties-in-great-britain-pedestrian-factsheet-2024) report published on 25th September 2025.
...
```
# 1. Main findings
Between 2004 and <!-- `r yr2calc` --> 2024:

 - fatalities were <!-- `r ud` --> down <!-- `r fat_cas_diff` --> 39% from <!-- `r NROW(fat_cas_2004)` --> 677 to <!-- `r NROW(fat_cas_TY)` --> 409
 - serious injuries (adjusted) decreased by <!-- `r round(ser_cas_diff)` --> 48%  
 - pedestrian traffic (distance walked) <!-- `r ped_miles_id` --> decreased by <!-- `r round(ped_traf*100)` --> 13%

Averaged over the period <!-- `r yr2calc-4` --> 2020 to <!-- `r yr2calc` --> 2024:
...
```
 ...
 - an average of <!-- `r round(mean(dths_per_wk_fat$casualties))` --> 8 pedestrians died and <!-- `r round(mean(dths_per_wk_ser$casualties, na.rm = TRUE))` --> 115 were seriously injured (adjusted) per week in reported road collisions
...
```
 ...
 - a majority of pedestrian fatalities (<!-- `r round(not_within_20$Fatalities)` --> 58%) do not occur at or within 20m of a junction compared to <!-- `r round(not_within_20$Serious)` --> 42% of all seriously injured (adjusted) casualties
...
```
 ...
 - <!-- `r round(single_car$pc_fat)` --> 66% of pedestrian fatalities were in collisions involving a single car
...
```
 ...
 - <!-- `r round(fatal_rural$pc)` --> 33% of pedestrian fatalities occurred on rural roads compared to <!-- `r round(rural_all$pc)` --> 14% of all pedestrian casualties
...
```
 ...
 - <!-- `r round(pc_fatal_serious_male$pc_ksi*100)` --> 65% of pedestrian killed or seriously injured (KSI) (adjusted) casualties were male
...
In <!-- `r yr2calc` --> 2024, <!-- `r NROW(fatal_TY)` --> 409 pedestrians were killed in Great Britain, whilst <!-- `r round(sum(serious_TY$casualty_adjusted_severity_serious,na.rm = TRUE))` --> 5965 were reported to be seriously injured and <!-- `r as.character(round(sum(slight_TY$casualty_adjusted_severity_slight, na.rm = TRUE)))` --> 13381 slightly injured.
...
Table 1 and Chart 1 show that pedestrian traffic (measured by distance walked) has <!-- `r dw` --> decreased between 2004 and <!-- `r yr2calc` --> 2024 whilst fatalities, serious and slight injuries have fallen.
...
Between <!-- `r yr2calc-1` --> 2023 and <!-- `r yr2calc` --> 2024, pedestrian fatalities <!-- `r pf` --> decreased by <!-- `r round(diff_fatal*100)` --> 3% while pedestrian traffic (distance walked) <!-- `r dw` --> increased by <!-- `r round(diff_trips*100)` --> 1%.
...
The pedestrian casualty rate has <!-- `r pcr` --> fallen for all severities in <!-- `r yr2calc` --> 2024 compared to 2004.
...
The overall casualty rate decreased by <!-- `r round(diff_all_2004)` --> 53%. The fatality rate decreased by <!-- `r round(diff_fat_2004)` --> 30% compared to a <!-- `r round(diff_sev_2004)` --> 41% reduction for serious injuries and a <!-- `r round(diff_sli_2004)` --> 57% reduction for slight injuries.
...
```
Between <!-- `r yr2calc-4` --> 2020 and <!-- `r yr2calc` --> 2024, <!-- `r round(male_tot$pc*100)` --> 65% of pedestrian casualties were male and <!-- `r round(female_tot$pc*100)` --> 35% female.

There are <!-- `r round(male_times, 1)` --> 1.9 times more male than female pedestrian casualties overall. This compares to <!-- `r round(sac_ab1$All[3]/sac_ab1$All[2],1)` --> 2.3 more for <!-- `r ab1` --> 30-39,  <!-- `r round(sac_ab2$All[3]/sac_ab2$All[2],1)` --> 1.5 more for <!-- `r ab2` --> 0-11 and <!-- `r round(sac_ab3$All[3]/sac_ab3$All[2],1)` --> 0.9 more for <!-- `r ab3` --> 70+ - the only age group in which female casualties outnumber males.
...
Between <!-- `r yr2calc-4` --> 2020 and <!-- `r yr2calc` --> 2024, most pedestrian fatalities occurred in  <!-- `r all_vehicles$number_of_vehicles[1]` --> 1 vehicle collisions involving  a <!-- `r all_vehicles$summary_group[1]` --> car (<!-- `r all_vehicles$Fatal[1]` --> 275).

However, the highest proportion of casualties from single vehicle collisions involve <!-- `r most_fat$summary_group[1]` --> 1 heavy goods vehicle (<!-- `r most_fat$pc_fat[1]` --> 4.8%). The second highest proportion (<!-- `r most_fat$pc_fat[2]` --> 3.3%) occurred in collisions when <!-- `r most_fat$summary_group[2]` --> 3 or more other vehicles involved.
...
The weekday peak time for pedestrian KSIs is from <!-- `r min(MF_peak$hr[1:4])` --> 3pm to <!-- `r max(MF_peak$hr[1:4])` --> 6pm. By contrast, the peak is from <!-- `r min(SS_peak$hr[1:4])` --> 1am to <!-- `r max(SS_peak$hr[1:4])` --> 9pm at weekends.
...
Chart 5 shows that between <!-- `r yr2calc-4` --> 2020 and <!-- `r yr2calc` --> 2024, <!-- `r round(fatal_urban$pc)` --> 63% of pedestrian fatalities occurred on urban roads compared to <!-- `r round(all_cas$pc)` --> 85% of all pedestrian casualties. <!-- `r round(fatal_mway$pc)` --> 4% of pedestrian fatalities occurred on motorways. This would be people outside their vehicles whether they are moving at the time or not.
...
A majority of pedestrian fatalities (<!-- `r highest_fat$Fatalities[1]` --> 58.4%) occur <!-- `r tolower(highest_fat$Junction[1])` --> not at junction or within 20 metres compared to <!-- `r highest_sev$Serious[1]` --> 42.1% of serious injuries (adjusted). However, <!-- `r round(sum(junc$Fatalities))` --> 29% of fatalities occur at a junction compared to <!-- `r round(sum(junc$Serious))` --> 43% of serious injuries (adjusted). Pedestrian fatalities at roundabouts represent <!-- `r round(sum(roundabouts$Fatalities))` --> 3% of all fatalities in contrast to <!-- `r round(sum(roundabouts$Serious))` --> 6% of serious injuries (adjusted).
