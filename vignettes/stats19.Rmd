...
For example, `read_collisions(format = TRUE)` converts values in the `accident_severity` column from `1`, `2` and `3` to `Slight`, `Serious` and `Fatal` using `fromat_accidents()` function.
...
crashes_2017_raw = read_collisions(year = 2017, format = FALSE)
...
By default, `format = TRUE`, meaning that the two stages of `read_collisions(format = FALSE)` and `format_accidents()` yield the same result as `read_collisions(format = TRUE)`.
