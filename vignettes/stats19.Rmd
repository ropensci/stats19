...
You can also use `years` and `type` to "search" through the file names, which are stored in a lazy-loaded dataset called `stats19::file_names`.
...
`format_accidents()` function automates the process of matching column names with variable names and labels in a [`.xls` file](http://data.dft.gov.uk/road-accidents-safety-data/Road-Accident-Safety-Data-Guide.xls) provided by the DfT.
...
cas_types$accident_index = as.character(cas_types$accident_index)
crashes_joined = left_join(crashes_wy, cas_types)
```
...
base::setdiff(names(crashes_joined), names(crashes_wy))
```
...
crashes_cycling = crashes_joined[crashes_joined$cycling > 0, ]
crashes_passenger = crashes_joined[crashes_joined$passenger > 0, ]
plot(crashes_cycling["speed_limit"], cex = crashes_cycling$Total)
plot(crashes_passenger["speed_limit"], cex = crashes_cycling$Total)
```
...
In addition to the `Total` number of people hurt/killed, `crashes_joined` contains a column for each type of casualty (cyclist, car occupant, etc.), and a number corresponding to the number of each type hurt in each crash.
...
library(ggplot2)
crashes_types = crashes_joined %>% 
  filter(accident_severity != "Slight") 
crashes_types$type = "Other"
crashes_types$type[crashes_types$walking > 0] = "Walking"
crashes_types$type[crashes_types$cycling > 0] = "Cycling"
crashes_types$type[crashes_types$passenger > 0] = "Passenger"
...
Another way of visualising these data is to show the spatial distribution by crash severity. This can be done with **ggplot2** as follows:

```{r ggplot-ped-severity}
ggplot(crashes_joined, aes(colour = Total)) +
  geom_sf() +
  facet_grid(vars(accident_severity))
```
...
```{r crash-date-plot}
d = lubridate::dmy(crashes_joined$date)
ud = unique(d)
crash_dates = data.frame(
  date = ud,
  n = c(
    aggregate(walking ~ d, sum, data = crashes_joined)[, 2],
    aggregate(cycling ~ d, sum, data = crashes_joined)[, 2],
    aggregate(passenger ~ d, sum, data = crashes_joined)[, 2]
),
  mode = rep(c("walking", "cycling", "passenger"), each = length(ud))
)
ggplot(crash_dates, aes(date, n)) +
...
```{r crash-time-plot}
library(stringr)
h = as.numeric(str_sub(crashes_joined$time, 1, 2))
uh = 1:24
crash_times = data.frame(
  hour = uh,
  n_casualties = c(
    aggregate(walking ~ h, sum, data = crashes_joined)[, 2],
    aggregate(cycling ~ h, sum, data = crashes_joined)[, 2],
    aggregate(passenger ~ h, sum, data = crashes_joined)[, 2]
  ),
  mode = rep(c("walking", "cycling", "passenger"), each = 24)
)
ggplot(crash_times, aes(hour, n_casualties)) +
