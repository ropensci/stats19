...
What just happened? We found the subset of casualties that took place in West Yorksire with reference to the `accident_index` variable.
...
The result is a spatial (`sf`) data frame of crashes in West Yorksire, with columns counting how many road users of different types were hurt.
...
Crashes tend to happen on busy Motorway roads and city centres, of Leeds and Bradford in particular.
This can be seen by comparing the previous map with an overview of the area, from [@lovelace_who_2016]:
...
```{r sfplot, fig.show='hold', out.width="100%", fig.cap="Spatial distribution of serious and fatal crashes in West Yorkshire in which pedestrians and car passengers were hurt. Colour is related to the speed limit where the crash happened (red is faster) and size is proportional to the total number of people hurt in each crash (legend not shown).", fig.width=9, fig.height=7}
library(ggplot2)
crashes_types = crashes_joined %>% 
  filter(accident_severity != "Slight") %>% 
  filter(Pedestrian > 0 | `Car occupant` > 0) %>% 
...
It is clear that pedestrians and car occupants tend to get hurt in different places.
Car occupant casualties, for example, are comparatively common on the outskirts of cities such as Leeds, where speed limits tend to be higher and where there are comparatively higher volumes of motor traffic, compared with the city centre.
