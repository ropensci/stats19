...
In all cases, a default parameter `read_*(format = TRUE)` returns the data in
formatted form, as described above. Data can also be imported in the
form directly provided by the DfT by passing `format = FALSE`, and then
subsequently formatted with additional `format_*()` functions, as described in a
final section of this vignette. Each of these `read*()` functions is now
...
The dataset contains `r ncol(crashes_2017_raw)` columns (variables) for `r format(nrow(crashes_2017_raw), big.mark = ",")` crashes.
...
For the full list of columns, run `names(crashes_2017)` or see the vignette.
...
```{r ukboundaries, eval = FALSE}
devtools::install_github("Robinlovelace/ukboundaries")
```
```{r ukboundaries-save, echo = FALSE, eval = FALSE}
leeds <- ukboundaries::leeds
saveRDS (leeds, file = "leeds-bdry.Rds")
```


```{r crashes-leeds, eval = FALSE}
library(sf)
...
nrow(crashes_leeds)
```
```{r crashes-leeds-hidden, echo = FALSE}
library (sf)
leeds_osgb = st_transform(readRDS("leeds-bdry.Rds"), crs = 27700)
crashes_leeds = crashes_sf[leeds_osgb, ]
nrow(crashes_sf)
nrow(crashes_leeds)
```

This subsetting has selected the `r format(nrow(crashes_leeds), big.mark = ",")`
crashes which occurred within the Leeds boundary.
...
crashes_joined
```
This join operation has enabled us to georeference the casualty data, enabling
us to explore the spatial distribution of different casualty types as follows:
...
These figures suggest that pedestrian casualties tend to happen more frequently near the city centre, compared with other types of casualty.
...
As mentioned above, it is also possible to import the "raw" data in precisely the format provided by the DfT.
