...
**stats19** enables access to and processing of Great Britain's official road traffic casualty database, [STATS19](https://www.data.gov.uk/dataset/cb7ae6f0-4be6-4935-9277-47e5ce24a11f/road-accidents-safety-data). 
...
**Note**: The Department for Transport used to refer to 'accidents', but "crashes" may be a more appropriate term, as emphasised in the "crash not accident" arguments of road safety advocacy groups such as [RoadPeace](https://www.roadpeace.org/working-for-change/).
...
For example, `read_collisions(format = TRUE)` converts values in the `collision_severity` column from `1`, `2` and `3` to `Slight`, `Serious` and `Fatal` using `fromat_collisions()` function.
...
The following code chunk, for example, downloads a file containing STATS19 data from 2022:
...
Currently, these files are downloaded to a default location of `tempdir` which is a platform independent "safe" but temporary location to download the data in.
The `dl_stats19()` function prints out the location and final file name(s) of files(s) as shown above.
...
The code below downloads and reads-in the 2022 crash data:
...
### Longitudinal trends and severity adjustments

When conducting long-term trend analysis (2004–2025), changes in police injury recording systems (notably the transition of English and Welsh forces to CRASH and COPA between 2016 and 2019) caused an artificial upward shift in recorded "Serious" casualties relative to "Slight" casualties.

To allow consistent longitudinal comparison over time, the Department for Transport provides modeled adjustment probabilities. We can use [`summarise_adjusted_severities()`](file:///home/robin/github/ropensci/stats19/R/adjustments.R) to compute and plot both unadjusted and adjusted time-series:

```{r time-series-adjustments, eval=FALSE}
# Fetch historical casualty data and calculate yearly adjusted severities
cas_historical = get_stats19(year = 2004:2025, type = "casualty")
cas_adj = summarise_adjusted_severities(cas_historical, by = "collision_year")

# Compare unadjusted vs DfT adjusted serious casualties
ggplot(cas_adj, aes(x = collision_year)) +
  geom_line(aes(y = serious_unadjusted, colour = "Unadjusted"), linetype = "dashed") +
  geom_line(aes(y = serious_adjusted, colour = "DfT Adjusted"), linewidth = 1) +
  labs(x = "Year", y = "Serious Casualties", colour = "Series",
       title = "GB Serious Casualties (2004–2025): Unadjusted vs DfT Adjusted") +
  theme_minimal()
```
