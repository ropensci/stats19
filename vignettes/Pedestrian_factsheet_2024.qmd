...
library(tidyr)
  ...
  mutate(fatal_count = 1)

  # same for non fatal
  other_cas <- casualties |>
  filter(!casualty_severity == "Fatal") |>
  mutate(fatal_count = 0)

  casualties <- rbind(fat_cas,other_cas)
...
```
# Introduction
This document aims to reproduce the `r yr2calc-4`-`r yr2calc` sections of the ["Accredited official statistics Reported road casualties in Great Britain: pedestrian factsheet, 2023"](https://www.gov.uk/government/statistics/reported-road-casualties-great-britain-pedestrian-factsheet-2024/reported-road-casualties-in-great-britain-pedestrian-factsheet-2024) report published on 25th September 2025.
...
diff_all_2004 <- (1-(rates_2$All[20]/rates_2$All[1]))*100
diff_fat_2004 <- (1-(rates_2$Fatal[20]/rates_2$Fatal[1]))*100
diff_sev_2004 <- (1-(rates_2$Serious[20]/rates_2$Serious[1]))*100
diff_sli_2004 <- (1-(rates_2$Slight[20]/rates_2$Slight[1]))*100
...
#   filter(!road_class == "Data missing or out of range") 
  # group_by(collision_severity,road_type) %>%
