...
#### Live Documentation & External Posts

> **Note**: This is a live document documenting recent updates, package evolution, and links to published external blog posts.
>
> - The original rOpenSci guest blog post introducing the package (April 2019) is published at [ropensci.org/blog/2019/04/16/stats19](https://ropensci.org/blog/2019/04/16/stats19/).
> - A forthcoming post covering stats19 v4.0.0+ (unified schema, DuckDB integration, and Parquet caching) will be published separately and linked here.

#### Recent Developments: Modernised Architecture (v4.0.0 - v4.2.0)

Road safety research in the UK relies heavily on STATS19—the official record of every reported road traffic collision. As the Department for Transport (DfT) modernized its data delivery, `stats19` has evolved alongside it to provide faster, cleaner, and more robust workflows for longitudinal research across 1979–2025.

##### 1. Unified Longitudinal Schema
Historical DfT releases varied substantially across years (e.g. `carriageway_hazards` vs `carriageway_hazards_historic`, `accident_reference` vs `collision_ref_no`, `accident_index` vs `collision_index`). In recent versions, `stats19` adopts a **Unified Longitudinal Schema**:
- "Historic" columns are automatically detected and merged into their modern equivalents.
- Reference numbers and indices are unified across tables.
- Researchers can analyze almost 50 years of data (1979–2025) using a single, consistent schema.

##### 2. Zero-Warning, High-Precision Parsing
- `read_stats19()` builds custom column parsers dynamically based on the exact CSV header of each file, eliminating unmatched parser warnings.
- Full floating-point precision is preserved for coordinates across all releases.
- Missing values (`-1`, "Code deprecated", "Data missing") are standardized to `NA` globally.

##### 3. Native DuckDB and Parquet Support
To handle multi-gigabyte historical data efficiently without memory bottlenecks, `stats19` now supports DuckDB and Parquet:

```r
library(stats19)

# 1. Query raw CSVs with DuckDB and SQL filters (e.g. 50x-75x faster on large files):
crashes_30mph = get_stats19(
  year = 2024,
  type = "collision",
  engine = "duckdb",
...
# 2. Convert downloaded CSV archives into compressed, typed Parquet files:
# stats19_to_parquet(type = "all")

# 3. Read directly from local Parquet cache:
crashes_pq = get_stats19(year = 2024, type = "collision", engine = "parquet")

# 4. Return lazy DuckDB connection for out-of-core aggregation with dbplyr:
crashes_db = get_stats19(year = 2024, type = "collision", output_format = "duckdb")
# crashes_db |> dplyr::count(collision_severity) |> dplyr::collect()
```

##### 4. Research Tools: Costs and Vehicle Cleaning
- `match_tag()`: Directly joins government TAG (Transport Analysis Guidance) economic cost estimates to collision data by severity and road category.
- `clean_make()`, `clean_model()`: Standardizes over 2,400 raw vehicle make/model strings.

---

#### Introduction & Package History

`stats19` is an R package that enables access to and processing of
...
1.  The release of the road crash statistics, which showed
...
for example (this page provides an excellent introduction to
STATS19).
...
(note the capital letters). There is another document called [STATS20](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/995423/stats20-2011.pdf), which is the guidance to STATS19 officers filling in the STATS19 form. 

An important point is that the dataset omits crashes in which
nobody was hurt.
The Department for Transport (DfT) also names the dataset [STATS19 on the main web page that links to open access road crash data](https://www.data.gov.uk/dataset/cb7ae6f0-4be6-4935-9277-47e5ce24a11f/road-accidents-safety-data).
...
There is a schema covering these tables but a good amount of work is needed to understand it, let alone be able to analyze the data contained within the files and convert the integers they contain into meaningful data.
...
We can combine the three sets of tables to analyse the data further. Let's read the datasets first:
...
Let's now read in casualties that took place in West Yorkshire (using `crashes_wy` object above), and count the number of casualties by severity for each crash:
