...
#library(stats19)
devtools::load_all("D://onedrive/github/stats19/")
library(sf)
library(dplyr)
library(lubridate)
library(reshape2)
...
if (yr2calc > 2023) {
  cat("A majority of pedestrian fatalities(", round(highest_fat$Fatalities[1]), "%) occur ",tolower(highest_fat$Junction[1])," compared to ", round(highest_sev$Serious[1]),"% of serious injuries (adjusted). However, ", round(sum(junc$Fatalities)), "% of fatalities occur at a junction compared to ", round(sum(junc$Serious)),"% of serious injuries (adjusted).") 
} else {
  cat("A majority of pedestrian fatalities", round(highest_fat$Fatalities[1]), "%) occur ",tolower(highest_fat$Junction[1])," compared to ", round(highest_sev$Serious[1]),"% of serious injuries (adjusted). However, ", round(sum(junc$Fatalities)), "% of fatalities occur at a junction compared to ", round(sum(junc$Serious)),"% of serious injuries (adjusted). Pedestrian fatalities at roundabouts represent ", round(sum(roundabouts$Fatalities)), "% of all fatalities in contrast to ", round(sum(roundabouts$Serious)),"% of serious injuries (adjusted).") 
}
```
