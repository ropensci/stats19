...
schame_dft = readxl::read_excel(schema_saved)
...
read_schema = function(
  ...
  filename = "Road-Safety-Open-Dataset-Data-Guide.xlsx",
  sheet = NULL
) {
  ...
  if(is.null(sheet)) {
    ...
    #                                     sheet = 2,
    #                                     skip = 2)
    export_variables = readxl::read_excel(file_path)
...
destfile = file.path(data_dir = stats19::get_data_directory(), schema_f)
...
variable_types
x = names(stats19::accidents_sample)
n = stats19_vtype(x)
names(n) = x
n
```
