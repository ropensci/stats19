...
<!-- badges: start -->
[![Open in GitHub Codespaces](https://img.shields.io/badge/Open%20in-GitHub%20Codespaces-blue?logo=github.png)](https://github.com/codespaces/new/ropensci/stats19?quickstart=1)
...
The following command, for example, gets crash data from 2023 (**note**: we follow the "crash not accident" campaign of [RoadPeace](https://www.roadpeace.org/working-for-change/) in naming crashes, and also adopted by the DfT since 2025):
...
We were not asked to download the file (by default you are not asked to confirm the file that will be downloaded).
...
```
To download other years it is worth being aware of the file structure of [the files, hosted by the DfT](https://www.gov.uk/government/statistical-data-sets/road-safety-open-data), this package imports. The last 5 years of data (usually released September for the previous year's full data) are available as individual years, or 1 file covering all 5 years. The data goes all the way back to 1979, the the longest running large national road traffic casualty datasets in the world, to the best of our knowledge (if you know of any other similar datasets please let us know in a new GitHub issue)! This data is stored in one very large file (~1GB) and also covers the last 5 years.

To request years using get_stats19 the following call structure can be used:
```{r requesting_years1, eval=FALSE}
...
```{r requesting_years2, eval=FALSE}
...
```{r requesting_years3, eval=FALSE}
...
```
But be aware, even though the two calls above are very different ranges, because they are both earlier than the last 5 years, the full raw file will be downloaded to your temporary directory, which can take sometime. Future calls should use this same file, but get_stats19 also formats this local file, which can still take some time. So think carefully about how you intend to work with the data.

## Working with large datasets

The full series, 1979 to 2025, is around 1.5 GB of CSV. Two options make it quicker to work with:

- Set `engine = "duckdb"` to filter with SQL inside DuckDB before the data reaches R.
- Convert the CSV files you have already downloaded into Parquet with `stats19_to_parquet()`, then read them back with `engine = "parquet"`. Parquet files are compressed and columnar, so repeated queries are faster and use less memory.

```{r parquet_workflow, eval=FALSE}
...
Parquet caches are not rebuilt automatically, so run `stats19_to_parquet()` again after downloading new years. See the [DuckDB and Parquet engines vignette](https://docs.ropensci.org/stats19/articles/duckdb.html) for the full workflow, including the `STATS19_PARQUET_DIRECTORY` environment variable.
...
What just happened? We found the subset of casualties that took place in West Yorkshire with reference to the `collision_index` variable.
