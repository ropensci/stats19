...
This work was done by `read_accidents()`, which imported the 'raw' Stats19 data without cleaning messy column names or re-categorising the outputs.
...
The results show that there were `r nrow(casualties_2017_raw)` casualties reported by the police in the STATS19 dataset in 2017.
There are `r ncol(casualties_2017_raw)` columns (variables) on these casualties.
...
The results show that there were `r nrow(vehicles_2017_raw)` vehicles involved in crashes reported by the police in the STATS19 dataset in 2017.
There are `r ncol(vehicles_2017_raw)` columns (variables) on these vehicles.
Values for a sample of these columns is shown below:
...
Another way of visualising the data, in this case to show the spatial distribution of crashes causing pedestrian casualties by numbers of pedestrians hurt, is with **ggplot2**:
