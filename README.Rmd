...
The following command, for example, gets crash data from 2017 (**note**: we follow the "crash not accident" campaign of [RoadPeace](http://www.roadpeace.org/take-action/crash-not-accident/) in naming crashes, although the DfT refers to the relevant tables as 'accidents' data):
...
Crashes tend to happen on busy Motorway roads (with high speed limits, as shown in the map above) and city centres, of Leeds and Bradford in particular.
The severity and number of people hurt (proportional to circle width in the map above) in crashes is related to the speed limit roads.
This can be seen by comparing the previous map with an overview of the area, from [@lovelace_who_2016]:

```{r, echo=FALSE}
knitr::include_graphics("https://raw.githubusercontent.com/ITSLeeds/stats19/master/vignettes/wy-overview.jpg")
```
