...
For the analysis of temporal data we will use the R package `lubridate` so, first of all, we need to load it
...
(x_date <- parse_date_time(x, "ymd"))
```

```{r}
year(x_date)
month(x_date, label = TRUE, abbr = FALSE)
```


Create new variable for crashes with datetime object.

Add year, month, day of week, hour/minute columns with lubridate
