...
Now we'll take a look at the time components of a Date. Using the function `hms` (acronym for Hour Minutes Seconds) and its subfunctions such as `hm` or `ms`, we can parse a character vector representing several times as an Hour object (which is called a Period object). 

```{r}
x <- c("18:23:35", "00:00:00", "12:34:56")
(x_time <- hms(x))
```
...
Subset based on time of day

Idea for bonus, with this answer not shown:

```{r crash times, eval=FALSE, echo=FALSE}
library(stats19)
a = stats19::get_stats19(year = 2017, type = "ac")
head(a$date)
head(a$time)
a$time_posix = lubridate::hm(a$time)
a$hour = lubridate::hour(a$time_posix)
a %>% 
  filter(hour == 1)
crashes_by_hour = a %>% 
  group_by(hour) %>% 
