...
1. Subset the crashes so that only the first and third columns of the data frame are returned.
1. Return the 2^nd^ row and the 3^rd^ column of the `crashes` dataset. 
1. Return the 2^nd^ row and the 2^nd^ *and* 3^rd^ column of the `crashes` dataset. 
...
R has lots of functionality built in, but many more functions are available in packages. 
Packages provide additional functions, data and documentation that is not available in the basic R installation.
There are two stages to using a package: installing/updating it and loading it.
...
Another useful package is `ggplot2`, a generic plotting package that is part of the ['tidyverse'](https://www.tidyverse.org/) meta-package, which is an "opinionated collection of R packages designed for data science". 
...
Another useful operator, that is part of `dplyr`, one of the most important packages in the tidyverse, is ` %>% `. 
...
1. Try installing a new package of you choice beginning with `gg`. Hint: enter `install.packages(gg)` and hit Tab when your cursor is between the `g` and the `)`.
1. Call a function from the new package without loading it using the `package_name::function()` syntax.
1. Load the package.
1. Get help on the function using `?function`.

# Temporal data

For the analysis and manipulation of temporal data we will use the R package `lubridate` so, first of all, we need to load it
...
Nevertheless, when you get a new dataset to work with or you update your database, the date variables are usually stored simply as a character vectors. This is a problem, since R is not always smart enough to distinguish between character vectors representing Dates and Dates objects:

```{r, eval=FALSE}
yday("2019-10-14") # works
weekdays("2019-10-14") #returns error
```

When you import your data into R, usually the import routines (such as `read.csv` or `read_csv`, look at the corresponding help page if you don't know those functions) recognize the Date variables (using several heuristics) and trasform them as Date objects, but this is not always the case (especially with particular date and time formats). So, we must be able to manually process Date objects. For example, given this vector of strings

```{r}
x <- c("2009-01-01", "2009-02-02", "2009-03-03")
```

then we can trasform it as a Date object, now called `x_date`, using the function `as.Date`. Notice that we must specify the format of these dates using the string `"%Y-%m-%d"`. 
...
1. Read the help page of `as.Date` and `strptime` for further details on the format argument. 
1. Extract the day, the year-day, the month and the weekday (as a non-abbreviated character vector) of each element of `x_date`. 
1. Modify the previous example to parse the following character string: `"09/09/93"` and extract its weekday. 
...
weekdays(as.Date("09/09/93", format = "%d/%m/%y"))
wday(as.Date("09/09/93", format = "%d/%m/%y"))
```
...
```{r}
filter(crashes, day(casualty_day) < 7) # the events occurred in the first week of the month
filter(crashes, weekdays(casualty_day) == "Monday") # the events occurred on monday
```

Exercises: 

1. Select only the events occurred in January
1. Select only the events occurred in an odd year-day 
1. Select only the events occurred in a leap-year (HINT: check the function `leap_year`)
1. Select only the events occurred during the weekend or in June
1. Select only the events occurred during the weekend and in June
1. Count how many events occurred during each day of the week. 
...
```{r}
crashes$casualty_hour <- hms(c("18:23:35", "00:00:01", "12:34:56"))
```

Exercises: 

1. Filter only the events occurred after midday (i.e. the PM events). You should manually code this exercises and, then, look at the help page of the function `am`. Moreover, if you simply type `am` in the console, then you can look at the definition of the function. 
1. Filter only the events occurred between 15:00 and 19:00
1. Round all hours to the next hour. Hint: Look at the help page of the `round_date` function. 
1. Bonus (very difficult): run the following code, which downloades data for car crashes occurred during 2017.  
...
Extract the weekday from the variable called `date` and calculate how many crashes occurred for each day of the week. Then plot it with ggplot2. Repeat the same exercises extracting the hour of the car accident from the variable called time. How would you combine the two informations in a single plot? 
