...
1. What are it's column names?
1. Print its first 2 features and rows 1 and columns 6:8 (the result is below).
...
five_in_name = zones[grepl(pattern = "5", x = zones$geo_name), ] # subset based on string match
...
<!-- ```{r} -->
<!-- st_crs(zones) # 4326 is a geographic (lon/lat) projection -->
<!-- zones_projected = st_transform(zones, 27700) # transfrom to projected CRS -->
<!-- st_crs(zones_projected) # updated CRS -->
<!-- zones_lonlat = st_transform(zones, 4326) # transfrom back -->
<!-- ``` -->
<!-- **Warning** It is possible to change the CRS without reprojecting the data as follows (not recommended): -->
<!-- ```{r, eval=FALSE} -->
<!-- st_crs(zones_projected) = 4326 -->
<!-- ``` -->
...
<!-- Let's load some centroids and find out which are in differe `zones` areas. -->

<!-- ```{r, warning=FALSE, out.width="30%"} -->
<!-- zones_cents = pct::get_pct_centroids(region = "isle-of-wight", geography = "lsoa") # Load the zones_cents data -->
<!-- zones_cents2 = zones_cents[zones_2,] -->
<!-- plot(zones$geometry) -->
<!-- plot(zones_cents, col = "black", add = TRUE) -->
<!-- plot(zones_cents2, col = "red", add = TRUE) -->
<!-- ``` -->
...
<!-- ```{r, eval=FALSE} -->
<!-- st_intersects(zones_cents, zones_large) -->
<!-- ``` -->
...
<!-- ```{r, eval=TRUE} -->
<!-- zones_cents3 = zones_cents[zones_2, , op = st_disjoint] -->
<!-- ``` -->
