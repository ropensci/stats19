...
We also could not see the vignette when installing using `devtools::install_github(build_vignettes = TRUE`. But we can see the vignette if we intall locally.
...
A CONTRIBUTING is added now. Than you.
...
@mem48 answered this: cyipt/stats19 is actually not an R pacakge. It is a repo which is used for CyIPT project, it has different sources (UK DS), and usage so there is no current need to adapt the use to this package. Malcom is one of the contributors to this package.
